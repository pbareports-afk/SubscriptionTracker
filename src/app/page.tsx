          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden pb-4">
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-[11px] text-slate-400 uppercase tracking-widest bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    <th scope="col" className="px-6 py-4 font-bold">Service</th>
                    <th scope="col" className="px-6 py-4 font-bold">Status</th>
                    <th scope="col" className="px-6 py-4 font-bold">Time Remaining</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {sortedServices.map((service) => {
                    ...
                    return (
                      <tr key={service.id} className={`hover:bg-slate-50/50 transition-colors group relative border-l-[3px] ${statusConfig.borderClass}`}>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="font-bold text-slate-900 text-[15px] leading-tight mb-0.5">{service.title}</p>
                        ...
                        <td className="px-6 py-4">
                          <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider ${statusConfig.badgeBg}`}>
                        ...
                        <td className="px-6 py-4 min-w-[200px]">
                          <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-baseline gap-1">
                              <span className={`text-base font-black ${statusConfig.numberColor}`}>{daysLeft < 0 ? 0 : daysLeft}</span>
                              <span className={`text-xs font-semibold ${statusConfig.numberColor}`}>days left</span>
                            </div>
                            <span className="text-xs font-bold text-slate-400">{format(service.expiryDate, "MMM dd, yyyy")}</span>
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="px-3 py-1 bg-slate-50 border border-slate-200 rounded-lg text-[11px] font-bold text-slate-500 shadow-sm inline-block truncate max-w-[120px]">
                            ...
                            <div className="flex items-center mr-2 gap-3">
                              {(!service.renewMethod || service.renewMethod === 'URL') ? (
                                <button 
                                  ...
                                  className="text-blue-600 font-bold text-[13px] hover:text-blue-700 transition-colors"
                                >
                                  Renew &gt;
                                </button>
